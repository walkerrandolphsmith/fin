...
const GuidedSection = ({
  ...
  noMarginOffset = false,
}) =>
  createElement(
    as,
    {
      className: `Section relative ${noMarginOffset ? "" : "ml-16"} z-10 ${angleTop ? "Section--angleTop" : ""} ${angleBoth ? "Section--angleBoth" : ""} ${angleBottom ? "Section--angleBottom" : ""} ${padBottom ? "Section--padBottom" : ""} ${noPaddingOnPrint ? "Section--noPaddingOnPrint" : ""} ${noPadding ? "Section--noPadding" : "Section--paddingNormal"} ${noBottomPadding ? "Section--noBottomPadding" : ""} ${noTopPadding ? "Section--noTopPadding" : ""} ${printHidden ? "print:hidden" : ""}`,
    },
    <div
      className="Section__masked"
      style={
        {
          ["--sectionOverflow" as string]: allowOverflow
            ? "visible"
            : "hidden",
